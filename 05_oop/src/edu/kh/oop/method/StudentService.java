...
public class StudentService {
	...
	private String studentInform(Student s) {
		...
		// KH대학교 / 학번 / 성별 / 이름 / java / html
		return String.format("%s / %s / %c / %s / %d / %d", Student.schoolName, s.getStudentNumber(), s.getGender(),
				s.getName(), s.getJava(), s.getHtml());
	}

	/**
	 * 매개변수로 전달 받은 학생 객체의 이름 변경
	 * 
	...
	private void updateStudentName(Student s) {
		...
		System.out.printf("%s -> %s %n", beforeName, newName);
		
	}
	
	/** Java 역량이 얼마나 증가 또는 감소 했는지 정수로 입력 받고
	 * 학생의 Java 역량을 수정
	 * 단, 수정된 Java 역량은 최대값, 최소값 범위를 넘어가지 못하게 한다
	 * 
	 * ex) Java 역량이 120인 경우 100(최대값)으로 표현
	 * Java 역량이 -30인 경우 최소값 0(최솟값)으로
	 * @param s : std1 또는 std2 (학생 객체 주소)
	 */
	...
	private void updateJava(Student s) {
		...
		// [결과 출력 예시]
		// Java 역량 수정 완료
		// 50 -> 100 (100)      // 최댓값 범위 초과 X
		
		// [결과 출력 예시]
		// Java 역량 수정 완료
		// 50 -> 0 (-100)      // 최솟값 범위 초과 X
