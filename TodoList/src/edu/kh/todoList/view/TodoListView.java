...
public class TodoListView {
	...
	public TodoListView() {
		...
		try { // 객체 생성시 발생되는 예외를 여기서 모아서 처리할 예정
			// 부모 타입 참조 변수 = 자식 객체 참조
	...
	public void startView() {
		...
		do {
			try {
				...
				System.out.println("### 입/출력 관련 예외가 발생했다.");
				e.printStackTrace();
	...
	private void todoListFullView() {
		...
		// 반환 받은 map 요소 뜯어내기
		List<Todo> todoList = (List<Todo>) map.get("todoList"); // @노란줄
	...
	private void todoDetailView() throws NumberFormatException, IOException {
		...
		if (todoDetail == null) {
			System.out.println("### 입력한 인덱스 번호가 존재하지 않아용");
			return;
		}
	...
	private void todoAdd() throws IOException {
		...
		while (true) {
			...
			// 문자열 입력 받기
			String content = br.readLine(); // !wq라면 break;
